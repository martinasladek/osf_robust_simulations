...
```{r}
cell_ids <- c("design", "b", "n_bw", "n_preds", "target_skew", "target_kurt", "target_het", "n_het_pred", "het_shape")
```

Select only predictor of interest: 

```{r}
sim_list <- purrr::map(
  .x = sim_list, 
  .f = \(x) x |> dplyr::filter(lm_term == "x1:x2")
)
```
...
```{r}
sim_df_power_sum_overall <- sim_df |> 
  ...
  dplyr::mutate(
    plot_group = paste0(
      ...
      "het_shape: ", het_shape, "; target_het: ", target_het, "; n_het: ", n_het_pred
    ), 
