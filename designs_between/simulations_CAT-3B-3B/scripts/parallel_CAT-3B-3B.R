...
cat_3b3b_sfs <- function(
   # pop_i_gb1_1, pop_i_gb1_2, pop_i_gb1_3, # grabbed from the global environment, re-defined on each loop 
    n_bw, n_ratio){
  
  ns <- sample_n_from_ratio_bw3(n_bw = n_bw, n_ratio = n_ratio)
  #ns <- sample_n_from_ratio_bw3(n_bw = 71, n_ratio = 1)
  
  weight_col <- paste0("n_ratio_w_", n_ratio)
  #weight_col <- paste0("n_ratio_w_", 1)
...
for(i in start_pop_i:cat_3b3b_n_pop){
