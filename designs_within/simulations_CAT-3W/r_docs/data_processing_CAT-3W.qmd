...
sim_list <- purrr::map(
  .x = sim_list, 
  .f = \(x) x |> dplyr::filter(afex_t_term == "gw3-gw1")
)
```
...
sim_df_overall_sum <- sim_df |> 
  dplyr::select(all_of(cell_ids), "afex_nc_p", "afex_gg_p") |> 
  tidyr::pivot_longer(
...
sim_df_overall_sum |> 
  ...
  geom_line() + 
  scale_x_continuous(breaks = unique(sim_df_overall_sum$n_rm)) + 
  facet_grid(
...
sim_df_power_sum <- sim_df |> 
  dplyr::select(all_of(cell_ids), contains("p.value"), "afex_t_p") |> 
  tidyr::pivot_longer(
    cols = -all_of(cell_ids)
  ) |> 
  dplyr::mutate(
