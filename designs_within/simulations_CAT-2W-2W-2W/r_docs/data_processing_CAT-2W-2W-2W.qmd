...
sim_list <- purrr::map(
  .x = sim_list, 
  .f = \(x) x |> dplyr::filter(afex_nc_term == "gw1:gw2:gw3")
)
```
...
sim_df_bias_sum <- sim_df |> 
  dplyr::select(all_of(cell_ids), contains("int_b")) |> 
  tidyr::pivot_longer(-all_of(cell_ids)) |> 
  dplyr::mutate(
    b_diff = value - (b*-1) # adding (-1) here because functions for interaction betas return b in negative values. 
  )  |> 
  dplyr::group_by(across(c(all_of(cell_ids), name))) |> 
  dplyr::summarise(
