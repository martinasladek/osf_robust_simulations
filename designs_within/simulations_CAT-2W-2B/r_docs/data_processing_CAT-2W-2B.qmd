...
```{r}
cell_ids <- c("design", "b", "n_rm", "n_ratio", "target_vr", "target_skew", "target_kurt", "n_het_pred")
```
...
sim_list <- purrr::map(
  .x = sim_list, 
  .f = \(x) x |> dplyr::filter(afex_nc_term %in% c("between_group:gw"))
)
```
