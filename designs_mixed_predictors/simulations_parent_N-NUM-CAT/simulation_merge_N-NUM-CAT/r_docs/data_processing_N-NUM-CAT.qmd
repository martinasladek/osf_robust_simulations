...
```{r}
cell_ids <- c("design", "b", "n_bw", "n_ratio", "n_cont_preds", "n_cat_preds", "target_skew", "target_kurt", 
              "het_shape", "target_het", "target_vr", "n_het_pred")
```
...
saveRDS(
  sim_export,
  paste0(
    "../data/processed_data/sim_export_", "N-NUM-CAT_preds_1", ".rds"),
)
```
...
saveRDS(
  sim_export,
  paste0(
    "../data/processed_data/sim_export_", "N-NUM-CAT_preds_3", ".rds"),
)
```
...
saveRDS(
  sim_export,
  paste0(
    "../data/processed_data/sim_export_", "N-NUM-CAT_preds_8", ".rds"),
)
```
...
sim_export_list$`sim_export_N-NUM-CAT_preds_8.rds` <-  list(
  sim_export_list$`sim_export_N-NUM-CAT_preds_8_nr_1.3.rds`, 
  sim_export_list$`sim_export_N-NUM-CAT_preds_8.rds`
) |> 
  purrr::transpose() |> 
  purrr::map(dplyr::bind_rows)

sim_export_list$`sim_export_N-NUM-CAT_preds_8_nr_1.3.rds` <- NULL

saveRDS(sim_export_list, file = "../data/processed_data/sim_export_N-NUM-CAT_split.rds")

# sim_export_list_t <- sim_export_list |> 
#   purrr::transpose() |> 
#   purrr::map(dplyr::bind_rows)
# 
# saveRDS(sim_export_list_t, file = "../data/processed_data/sim_export_N-NUM-CAT.rds")
```
...
# my_list <- list(
#   pred_1 = list(
#     power_sum = data.frame(a = 1, b = 1),
#     cover_sum = data.frame(c = 1, d = 1)
#   ), 
#   pred_3 = list(
#     power_sum = data.frame(a = 3, b = 3),
#     cover_sum = data.frame(c = 3, d = 3)
#   )
# )
# 
# 
...
# 
# my_list_res <- list(
#   power_sum = rbind(data.frame(a = 1, b = 1), data.frame(a = 3, b = 3)), 
#   cover_sum = rbind(data.frame(c = 1, d = 1), data.frame(c = 3, d = 3))
# )
```
